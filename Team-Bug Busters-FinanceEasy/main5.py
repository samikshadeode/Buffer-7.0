...
#DSA USED:HASH MAPS,BINARY SEARCH,SLIDING WINDOW,GREEDY ALGORITHM,PREFIX SUM,HEAPS,KADANE'S ALGORITHM


#list of dicts to store budgets for each category
budgets_data = {
...
#lis of dicts to store expenses for each category
expenses = {
...
transaction_history.append({"category": "grocery", "amount": 60, "date": demo_date2})
transaction_history.append({"category": "rent", "amount": 70, "date": demo_date3})
transaction_history.append({"category": "tuition", "amount": 80, "date": demo_date4})
...
def budgets():
    ...
    with use_scope('output_area', clear=True):
        ...
        put_text("Click to configure each category:")
    ...
    with use_scope('output_area', clear=True):
        ...
        if not three_maxi:
            ...
            for expense in three_maxi:
                put_text("Category:"+expense["category"]+" Amount:"+str(expense["amount"]))
...
def search_transaction_by_date():
    ...
    date = input("Enter date to search transactions", type=DATE)
    ...
    # Keeping your binary search logic
    low=0
...
#ask suer to add goals and find monthly savings to achive that goal
def goals():
      global savings_amt
      ...
      put_text("You have to save:" + str(savings_per_month))
...
def savings():
   global total_saved
    ...
    # Use a popup for input to keep the main dashboard clean
   saved = input("Deposit to Savings", type=FLOAT, placeholder="Enter amount...")
   total_saved += saved
    
    # Toast notifications look much more "premium" than put_text
...
def main():
    ...
    put_column([
        ...
        put_button("Run Greedy Optimization", onclick=lambda: greedy(), color='success')
    ])
    ],size='25% 25% 25% 25%')
...
def calculate_transaction_between_2_dates():
    ...
    #if we conisder ust starting date then transactions ON that date arent considered
...
def spike():
        ...
        sorted_week=sorted(week_total.items(),key=lambda x:x[0]) #list of tuples sorted by week start date
             ...
             #compare this weeks spending with last 3 weeks average speinding and give user feedback
      ...
      #SLIDING WINDOW USED
        ...
        for i in range(3,len(sorted_week)):
                ...
                if(sorted_week[i][1]>weeks_avg*1.5): #if this week spending is more than 1.5 times the average of last 3 weeks spending, then show spike
                  put_text("You have a spike in expenses this week")
                else:
                  put_text("Congrats!You spent less than last week!")
      
        
       


#kadane's algorithm used to find most profitable days
